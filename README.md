...
1. Skill local-ai-use (✅ COMPLETADO) - Catálogo oficial AMD para ejecutar/optimizar LLMs
locales en hardware AMD
2. Dashboard IA (🟡 EN PROGRESO) - Interfaz web para gestionar modelos, benchmarks, autotuning y generación de scripts optimizados
Hardware Objetivo
...
detect_local.py
Detección GPU, validación backends, matriz
compatibilidad Setup inicial, CI/CD
bench_local.py Matriz parametrizable (hasta 1,620 combos) Comparación
rendimiento
tune_local.py Grid search 5 perfiles optimización Selección config
óptima
analyze_local.py
Escalabilidad, traces Chrome, Prometheus,
comparación Análisis profundo
...
│ ├ BENCHMARK RESULTS QWEN3 5 9B d # D t ll b h k
...
🔬 4. HALLAZGOS TÉCNICOS CLAVE (Validados
Experimentalmente)
...
512 294 3,346 100%
1024 395 3,368 67% ⭐
🎯 Conclusión Crítica: Para modelos ≤8GB Q4_K, CPU Zen 4 es 2.7-9x más rápido que GPU.
La GPU RX 9070 no está bien aprovechada por kernels actuales.
⚠️ Nota: HIP usa SDPA fallback en RDNA 4 (no Flash Attention nativo). Vulkan tiene FA nativo
pero kernels menos optimizados.
Batch Prefill tok/s Decode tok/s Eficiencia
2048 294 3,336 25%
4096 428 3,367 18%
...
Modelo Cuantización Tamaño Backend
Óptimo Config Recomendada
Qwen3.5-
9B Q4_K_XL 5.56 GB CPU (balanced) ngl=0, p=512, g=256,
b=2048, fa=off
Qwen3.5-
9B Q4_K_XL 5.56 GB HIP (latency) ngl=-1, p=2048, g=256,
b=1024, fa=auto
Qwen3.8-
27B Q3_K 12.24
GB HIP (balanced) ngl=-1, p=2048, g=256,
b=2048, fa=auto
🎯 Óptimo: batch=1024 (mejor eficiencia). Batch >2048 causa contención
memoria/sincronización.
🎯 Sweet spot: ngl=10-20 para híbrido. ngl=0 (CPU) es dramáticamente más rápido para
modelos pequeños.
...
ROCm DLLs no
en PATH
Añadir _rocm_sdk_devel\bin a PATH
permanentemente
invalid parameter:
-c
Flag ctx
incorrecta
Usar -p (prompt) y -n (gen), no -c / -
ctx-size
OOM
VRAM
insuficiente
Reducir n_gpu_layers , batch_size ,
usar cache_type q4_0
Flash Attention not
supported
FA no disponible
en RDNA 4 Usar -fa off o -fa auto (SDPA fallback)
Model not found
Path GGUF
incorrecto
Verificar path absoluto,
usar detect_local.py
...
 
python skills\local-ai-use\scripts\detect_local.py - validate-backends - compatibili
# O solo JSON para CI/CD:
python skills\local-ai-use\scripts\detect_local.py - validate-backends - compatibili
BASH
 
# Quick (32 combos, 2 reps)
python skills\local-ai-use\scripts\bench_local.py - model Qwen3.5-9B - backend hip -
# Full (1,620 combos, 3 reps)
python skills\local-ai-use\scripts\bench_local.py - model Qwen3.5-9B - backend both
BASH
Análisis
...
4 scripts + data +
evals + docs
detect_local.py ✅ VALIDADO skills/. /scripts/
--validatebackends --
compatibility
 
# Listar perfiles
...
python skills\local-ai-use\scripts\tune_local.py - model Qwen3.5-9B - backend hip -
# Throughput (servidor)
python skills\local-ai-use\scripts\tune_local.py - model Qwen3.5-9B - backend hip -
...
 
# Escalabilidad batch
python skills\local-ai-use\scripts\analyze_local.py - model Qwen3.5-9B - backend hip
...
BASH
Componente Estado Ubicación Notas
bench_local.py ✅ VALIDADO skills/. /scripts/
Quick + Full
matrix
tune_local.py ✅ VALIDADO skills/. /scripts/
5 perfiles +
presets.ini
analyze_local.py ✅ VALIDADO skills/. /scripts/
Scaling + traces
+ compare
Dashboard main.py
⏳ EN
...
CPU Zen 4 supera a GPU RX 9070 en 2.7-9x para modelos ≤8GB Q4_K. La GPU no está bien
aprovechada por kernels actuales. HIP > Vulkan en GPU (+20% decode, +55% prefill).
